...
import { categories as catDefs, items as catItems } from './pricebookData';
...
async function startup() {
  ...
  if (existingCount !== expectedCount) {
    ...
    for (const item of catItems) {
      ...
      if (!catId) { console.log(`[startup] WARN: no categoryId for ${item.sku} (${item.categoryKey})`); continue; }
      await prisma.pricebookItem.create({
        data: {
          ...
          volumePricing: JSON.parse(JSON.stringify(item.volumeTiers)),
          categoryId: catId,
        },
      });
      seededCount++;
