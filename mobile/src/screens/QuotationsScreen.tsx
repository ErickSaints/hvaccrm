...
const statusColors: Record<string, string> = { BORRADOR: '#6b7280', ENVIADA: '#8b5cf6', ACEPTADA: '#16a34a', RECHAZADA: '#ef4444' };
...
export default function QuotationsScreen({ navigation }: any) {
  ...
  const load = async () => {
    try {
      const { data } = await api.get('/proposals', { params: { limit: 50, page: 1, search: search || undefined } });
      setItems(data.data || []);
    } catch { /* silent */ }
  ...
  return (
    <View style={s.container}>
      <View style={s.header}><Text style={s.title}>Cotizaciones</Text></View>
      ...
      <FlatList data={items} keyExtractor={i => i.id.toString()}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={s.card} onPress={() => navigation.navigate('QuotationDetail', { id: item.id })}>
            <View style={s.row}>
              <Text style={s.titleText}>{item.customer?.contactName || `Cotización #${item.id}`}</Text>
