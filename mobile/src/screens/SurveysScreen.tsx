...
export default function SurveysScreen({ navigation }: any) {
  ...
  const load = async () => {
    try {
      const { data } = await api.get('/surveys', { params: { limit: 50, page: 1, search: search || undefined } });
      setItems(data.data || []);
    } catch { /* silent */ }
  ...
  return (
    <View style={s.container}>
      <View style={s.header}><Text style={s.title}>Levantamientos</Text></View>
      ...
      <FlatList data={items} keyExtractor={i => i.id.toString()}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={s.card} onPress={() => navigation.navigate('SurveyDetail', { id: item.id })}>
            <Text style={s.titleText}>Levantamiento #{item.id}</Text>
