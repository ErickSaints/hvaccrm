...
const statusColors: Record<string, string> = { BORRADOR: '#6b7280', APROBADA: '#2563eb', RECIBIDA: '#16a34a', CANCELADA: '#ef4444' };
const statusLabels: Record<string, string> = { BORRADOR: 'Borrador', APROBADA: 'Aprobada', RECIBIDA: 'Recibida', CANCELADA: 'Cancelada' };
...
export default function PurchaseOrdersScreen({ navigation }: any) {
  ...
  const load = async () => {
    try {
      const { data } = await api.get('/purchase-orders', { params: { limit: 50, page: 1, search: search || undefined } });
      setItems(data.data || []);
    } catch { /* silent */ }
  ...
  return (
    <View style={s.container}>
      <View style={s.header}><Text style={s.title}>Órdenes de Compra</Text></View>
      ...
      <FlatList data={items} keyExtractor={i => i.id.toString()}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={s.card}>
            <View style={s.row}>
              <Text style={s.titleText}>OC #{item.id}</Text>
            ...
            <Text style={s.provider}>{item.vendor?.name || 'Proveedor'}</Text>
...
const s = StyleSheet.create({
  ...
  provider: { fontSize: 14, color: '#6b7280', marginTop: 4 },
  amount: { fontSize: 16, fontWeight: '700', color: '#059669', marginTop: 4 },
