...
const statusColors: Record<string, string> = {
  ABIERTO: '#ef4444', EN_PROCESO: '#d97706', RESUELTO: '#16a34a', CERRADO: '#6b7280',
};
const statusLabels: Record<string, string> = {
  ABIERTO: 'Abierto', EN_PROCESO: 'En Proceso', RESUELTO: 'Resuelto', CERRADO: 'Cerrado',
};
const levelLabels: Record<string, string> = {
  EMERGENCIA: 'Emergencia', ATENCION: 'Atención', PROGRAMAR: 'Programar',
};
...
export default function TicketsScreen({ navigation }: any) {
  ...
  return (
    <View style={s.container}>
      <View style={s.header}><Text style={s.title}>Tickets</Text></View>
      <TextInput style={s.search} placeholder="Buscar ticket..." placeholderTextColor="#9ca3af" value={search} onChangeText={setSearch} />
      <FlatList data={items} keyExtractor={i => i.id.toString()}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={load} />}
        renderItem={({ item }) => (
          <TouchableOpacity style={s.card} onPress={() => navigation.navigate('TicketDetail', { id: item.id })}>
            <View style={s.row}>
              <Text style={s.titleText}>#{item.id} {item.title}</Text>
            ...
            <Text style={s.level}>{levelLabels[item.level]}</Text>
...
const s = StyleSheet.create({
  ...
  level: { fontSize: 13, color: '#6b7280', marginTop: 4 },
