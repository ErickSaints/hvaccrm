  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/assets" className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{asset.name}</h1>
          <p className="text-gray-500 mt-1">{asset.customer.companyName || asset.customer.contactName}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="card space-y-4">
            ...
            <p className="text-xs text-center text-gray-400 break-all">{qrUrl}</p>
            <a
              href={qrImage}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card">
            ...
            {asset.maintenanceLogs.length === 0 ? (
              <p className="text-gray-500 text-sm text-center py-4">Sin registros de mantenimiento</p>
            ) : (
              <div className="space-y-3">
                {asset.maintenanceLogs.map((log) => (
                  <div key={log.id} className="p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-gray-900 text-sm">{log.description}</p>
          ...
          <div className="card">
            ...
            {asset.serviceReports.length === 0 ? (
              <p className="text-gray-500 text-sm text-center py-4">Sin reportes de servicio</p>
            ) : (
                    ...
                    {report.photos.length > 0 && (
                      <div className="flex gap-2 mt-2 overflow-x-auto">
                        {report.photos.map((photo, i) => (
                          <img key={i} src={photo.url} alt={photo.caption || ''} className="w-16 h-16 object-cover rounded border border-gray-200" />
                        ))}
                      </div>
                    )}
                  </div>
