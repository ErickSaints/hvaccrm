    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 no-print">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/invoices')} className="p-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-800 rounded-lg transition-colors">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-gray-400 dark:text-gray-500">{invoice.number}</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{invoice.title}</h1>
          </div>
        </div>
      ...
      <div className="card print:shadow-none">
        <div className="text-center border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">{invoice.title}</h1>
          <p className="text-gray-500 dark:text-gray-400">Factura #{invoice.number}</p>
        </div>

        <div className="grid grid-cols-2 gap-6 mb-6">
          <div>
            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Cliente</h3>
            <p className="text-gray-900 dark:text-gray-100 font-medium">{invoice.customer?.contactName}</p>
            {invoice.customer?.companyName && <p className="text-gray-600 text-sm">{invoice.customer.companyName}</p>}
            {invoice.customer?.email && <p className="text-gray-500 dark:text-gray-400 text-sm">{invoice.customer.email}</p>}
          </div>
          <div className="text-right">
            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Detalles</h3>
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
          <div className="space-y-2 max-w-xs ml-auto">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-900 dark:text-gray-100">${invoice.subtotal.toLocaleString('es-MX')}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500 dark:text-gray-400">IVA</span>
              <span className="text-gray-900 dark:text-gray-100">${invoice.tax.toLocaleString('es-MX')}</span>
            </div>
            {invoice.discount > 0 && (
              <div className="flex justify-between text-sm">
            ...
            <div className="flex justify-between text-lg font-bold border-t border-gray-200 dark:border-gray-700 pt-2">
              ...
              <span className="text-gray-900 dark:text-gray-100">${invoice.total.toLocaleString('es-MX')}</span>
            </div>
          </div>
        </div>
